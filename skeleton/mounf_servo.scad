    ...
    module feet_box() {
        gear_mount();
        translate( [MG90S_FULL_SIZE.x/2-1,-MG90S_FULL_SIZE.y/2,7]) cube([5, MG90S_FULL_SIZE.y, 13]);
        
            translate([19.5,2,5]) rotate([20,-20,0])  union() {cylinder(h=20,d=8, center=true);
                    translate([0,0,-10]) sphere(d=8);
            }
        
            translate( [-MG90S_FULL_SIZE.x/2-14,-MG90S_FULL_SIZE.y/2,7]) cube([15, MG90S_FULL_SIZE.y, 13]);
        
            translate([-29.5,2,5]) rotate([20,20,0])  union() {cylinder(h=20,d=8, center=true);
                    translate([0,0,-10]) sphere(d=8);
            }
    }
    ...
    module mouf_sk(pitch=0, yaw=0, roll=0, double_arm=false) {
        ...
        {     // servo
            mg90s();
            translate( [0,0,-2]) {
                color("green") feet_box();
            }
            // arm
            translate([-(MG90S_BLOCK_SIZE.x/2-MG90S_FULL_SIZE.y/2),0,MG90S_FULL_SIZE.z]) rotate([0,0,roll]) servo_arm_dual();
            }
        ...
        translate([-10,0,0]){
            rotate([0,90,0]) {
                translate([-(MG90S_BLOCK_SIZE.x/2-MG90S_FULL_SIZE.y/2),0,MG90S_FULL_SIZE.z]) rotate([0,0,pitch]) {
                    ...
                    rotate([90,0,0]) translate([-40,-22,-10]) {
                        mg90s();
                        translate([-(MG90S_BLOCK_SIZE.x/2-MG90S_FULL_SIZE.y/2),0,MG90S_FULL_SIZE.z]) rotate([0,0,yaw]) {
                            servo_arm();    
                            rotate([0,90,0]) translate([-2,0,-14])  {
                                color("pink") head();
                            }
                        }
                    };
